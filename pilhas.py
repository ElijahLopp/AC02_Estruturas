...
def balanceada(string):
    ...
    for elt in string:
        ...
        if pilha_vazia(stack):
            stack.append(elt)
            count = 1
            print("if_pilhavazia"+str(stack))
        if encaixa(stack[t - 1], elt):
            stack.pop()
            print("if_encaixa"+str(stack))
        elif count == 0 and elt in "({[<":
            stack.append(elt)
            print("elif"+str(stack))
    if len(stack) == 0:
        fotografa(max_)
        return True
    return False
...
def palindromo(string):
    ...
    stack = []
    list_ = list(string)
    meeting = int(length/2)

    for enc in range(meeting):
        stack.append(list_[enc])

    if len(list_) == 0 or len(list_) == 1:
        return True

    if length % 2 != 0:
        if list_[meeting + 1] == stack[meeting - 1]:
            return True
        return False
    if stack[meeting - 1] == list_[meeting]:
        return True
    return False
...
class TestStringMethods(unittest.TestCase):
    ...
    def test_001_var_pilha1(self):
        self.verifica_pilha(
            pilha1, '30683e81e541f4f031f50134b6c4df4fb2325fb6222265fffa004832')

    def test_002_var_pilha2(self):
        self.verifica_pilha(
            pilha2, '12f16a9f7220b8fbba114ca53fa589d2137060e48ef51f87ede96a7d')

    def test_003_var_pilha3(self):
        self.verifica_pilha(
            pilha3, 'fc53f829aced039d8170455b2e1b24e1fa08bebfd5457257832257a4')
    ...
    def test_014_pilha_letras_verifica_fotos(self):
        reseta_fotos()
        pilha_letras('banana')
        album_correto = [['b'], ['b', 'a'], ['b', 'a', 'n'], ['b', 'a', 'n', 'a'], [
            'b', 'a', 'n', 'a', 'n'], ['b', 'a', 'n', 'a', 'n', 'a']]
    ...
    def test_016_menos_o_d_verifica_fotos(self):
        reseta_fotos()
        menos_o_d('bandana')
        album_correto = [['b'], ['b', 'a'], ['b', 'a', 'n'], ['b', 'a'], [
            'b', 'a', 'a'], ['b', 'a', 'a', 'n'], ['b', 'a', 'a', 'n', 'a']]
        album_execucao_da_funcao = fotografias
    ...
    def test_020_pilha_balanceada1(self):
        self.verifica_pilha(
            pilha_balanceada1, 'd05014e01b5592cef47539758e813503f58c743512164f6021ee62ac')

    def test_021_pilha_balanceada2(self):
        self.verifica_pilha(
            pilha_balanceada2, '53edf5c1b62d1568ba48e5f9ae4c65790a60a7084c91e6afbf600471')

    def test_023_pilha_balanceada3(self):
        self.verifica_pilha(
            pilha_balanceada3, '45ec24251c4b6f1be75226ba5bc896de21a0d4100de755ea870268e6')
    ...
    def test_028_balanceado_contabilizou_maximo(self):
        ...
        try:
            ...
            self.fail(
                'eu esperava uma fotografia, quando executei balanceada da string ((()))')
        reseta_fotos()
        balanceada('((()))')
    ...
    def verifica_pilha(self, pilha, codigo_correto):
        codigo_resp_aluno = hashlib.sha224(
            str(pilha).encode('utf-8')).hexdigest()
...
def explica_erro(album1, album2):
    ...
    if len(diferentes) > 0:
        ...
        print('Suas primeiras ' + str(menor_diferente) +
              ' fotografias estao certas')
    ...
    if len(album1) < len(album2):
        ...
        print('Voce tirou '+str(len(album1)) +
              ' fotografias, mas era pra tirar ' + str(len(album2)))
